...
def geo_train(device, x_in, y_in, xb, yb, ub, vb, xd, yd, ud, vd,
    ...
    torch.save(net2_u.state_dict(), os.path.join(path, "IA_data_u.pt"))
    torch.save(net2_v.state_dict(), os.path.join(path, "IA_data_v.pt"))
    torch.save(net2_p.state_dict(), os.path.join(path, "IA_data_p.pt"))
    ...
    ax_loss.set_title('Training Loss History - 2D IA PINN')
    ax_loss.legend()
    ...
    fig.suptitle(
        f'PINN - 2D Aneurysm  (nu={Diff}, Re~{int(U_scale/Diff)}, {epochs} epochs)',
...
#         otherwise falls back to CPU. CUDA supported on Linux/Windows.
# =============================================================================
...
Flag_BC_exact = False  # hard BC enforcement via output transform (not implemented 2D)
Lambda_BC     = 1.0    # weight for wall no-slip BC loss
Lambda_data   = 1.0    # weight for sparse velocity data loss

# -----------------------------------------------------------------------------
# Data paths: points to the local Data/2D-aneurysm/ directory.
# Update Directory if your files are elsewhere.
# -----------------------------------------------------------------------------
Directory    = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                            '..', 'Data', '2D-aneurysm') + os.sep
...
Flag_x_length = True   # if True, scale coordinates to [0,1] x [0,1] internally
X_scale = 3.0          # physical x-length of domain
Y_scale = 2.0          # physical y-length of domain
...
# Scale coordinates into [0,1] x [0,1]
...
# Inlet: parabolic profile  u = y*(0.2 - y)/0.01 * U_BC_in
# Wall : no-slip  u = v = 0
# =============================================================================
u_in_BC   = yb_in * (0.2 - yb_in) / 0.01 * U_BC_in  # parabolic inlet profile
...
# =============================================================================
x_data = np.asarray([1.8, 2.0, 1.5, 1.75, 2.1])
y_data = np.asarray([0.4, 0.5, 0.2, 0.9,  0.75])
